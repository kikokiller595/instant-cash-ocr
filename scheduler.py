...
def validate_latest_for_slot(expected_iso: str):
    ...
    ok = (
        is_final_digits(entry.get("pick2", ""), 2) and
        is_final_digits(entry.get("pick3", ""), 3) and
        is_final_digits(entry.get("pick4", ""), 4) and
        is_final_digits(entry.get("pick5", ""), 5)
    )
    if not ok:
...
def try_with_retries(run_at: datetime):
    ...
    log(f"FAIL draw_id={expected_iso} after 3 attempts • last_note='{last_note}' • last_exit={last_exit}")
